...
class Base():
    ...
    def eval_batch(self, batch, metrics, mode='val'):
        ...
        if mode == 'test':
            return metrics, batch['image_path'], labels, preds
        del output_dict, preds
        return metrics
        ...
        for epoch in range(start_epoch, self.CFG.EXP.NUM_EPOCHS):
            ...
            if val_acc > wandb.run.summary['best_val_acc']:
                ...
                for cls in self.class_names:
                    metric_key = 'val_acc_{}'.format
                    if metric_key in metrics:
                        wandb.run.summary['best_val_{}'.format(cls)] = metrics['val_acc_{}'.format(cls)].avg
                    else:
                        print("Warning Metric {} not in metrics".format(metric_key))
                        wandb.run.summary['best_val_{}'.format(cls)] = None  # Or handle as needed
    ...
    def test(self, test_dataloader, checkpoint_file):
        ...
        with torch.no_grad():
            for i, data in enumerate(tqdm(test_dataloader)):
                metrics, image_path, label, pred = self.eval_batch(
                    data,
                    metrics,
                    mode='test',
                )
                img_paths.append(image_path[0])
                labels_imgs.append(label[0])
                preds_img.append(pred[0])
        ...
        if self.CFG.EXP.APPROACH == 'coco_gender' or self.CFG.EXP.APPROACH == 'coco_abn' :
            ...
            self.confusion_m(img_paths, metrics, labels_imgs, preds_img, mode='test')
        
        # Confusion matrix and save results
        if self.CFG.DATA.DATASET=='waterbirds':
            compute_confusion_matrix_and_save(labels_imgs, preds_img, img_paths,
            output_dir='../GALS/eval', mode='test')
...
def compute_confusion_matrix_and_save(all_labels, all_preds, image_paths, output_dir, mode='test'):
    """
    Computes the confusion matrix, categorizes image paths into TP, TN, FP, FN, 
    and saves all results in the same text file.
    ...
    if isinstance(all_labels, list):
        # If the list is not empty, we ensure all labels are moved to CPU and converted to NumPy
        all_labels = np.array([label.cpu().numpy() if isinstance(label, torch.Tensor) else label for label in all_labels])
    if isinstance(all_preds, list):
        # Similarly, handle predictions
        all_preds = np.array([pred.cpu().numpy() if isinstance(pred, torch.Tensor) else pred for pred in all_preds])
    ...
    # Categorize images
    categorized_results = []
    for path, label, pred in zip(image_paths, all_labels, all_preds):
        if label == pred == 1:  # True Positive
            category = 'TP'
        elif label == pred == 0:  # True Negative
            category = 'TN'
            ...
            category = 'Unknown'  # Catch any unexpected case (optional)

        categorized_results.append(f'{path}, {category}, True Label: {label}, Predicted: {pred}\n')

    # Save results to a text file
    os.makedirs(output_dir, exist_ok=True)
    output_file = os.path.join(output_dir, f'{mode}_cub_image_id_tracking.txt')
    with open(output_file, 'w') as f:
        f.writelines(categorized_results)

    print(f'Confusion matrix and categorized results saved to {output_file}')
    return cm
