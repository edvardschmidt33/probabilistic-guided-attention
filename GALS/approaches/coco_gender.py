...
class COCOGenderCNN(GenericCNN):
    ...
    def confusion_m(self, data, metrics, labels, preds, mode='test'):
        ...
        for i,pred in enumerate(preds):
            ...
            if (female & pred_female):
                f_tp += 1
                image_id_tracking['female_tp'].append(img_id)
            if (male & pred_male):
                m_tp += 1
                image_id_tracking['male_tp'].append(img_id)
            if (male & pred_female):
                f_fp += 1
                image_id_tracking['female_fp'].append(img_id)
            if (female & pred_male):
                m_fp += 1
                image_id_tracking['male_fp'].append(img_id)
            if ((not female) & (not pred_female)):
                f_tn += 1
                image_id_tracking['female_tn'].append(img_id)
            if ((not male) & (not pred_male)):
                m_tn += 1
                image_id_tracking['male_tn'].append(img_id)
            if ((female) & (not pred_female)):
                f_fn += 1
                image_id_tracking['female_fn'].append(img_id)
            if ((male) & ( not pred_male)):
                m_tn += 1
                image_id_tracking['male_fn'].append(img_id)
            if (female & pred_other):
                f_other += 1
                image_id_tracking['female_other'].append(img_id)
            if (male & pred_other):
                m_other += 1
                image_id_tracking['male_other'].append(img_id)
            if female:
                f_total += 1
                image_id_tracking['female_total'].append(img_id)
            if male:
                m_total += 1
                image_id_tracking['male_total'].append(img_id)

        self.save_image_ids(image_id_tracking, mode)
        
    def save_image_ids(self, image_id_tracking, mode):
        """
        Save the tracked image IDs into a file.
        """
        save_path = f"../GALS/eval/{mode}_coco_image_id_tracking.txt"
        with open(save_path, "w") as f:
            for category, ids in image_id_tracking.items():
                f.write(f"{category}:\n")
                f.write(", ".join(map(str, ids)) + "\n")
        print(f"Image ID tracking saved to {save_path}")
