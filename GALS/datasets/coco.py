...
class COCOGender(torch.utils.data.Dataset):
    def __init__(self, root, cfg, split='train', transform=None, size=None, compute_all=False):
        ...
        """
        coco = COCO(
            'data/COCO/COCO_gender/filtered_instances_{}2014_shift.json'.format(
                'train' if split == 'train' else 'val'
            )
        )"""
            ...
            self.attention_data = np.array(
                [
                    os.path.join(
                        self.root,
                        'COCO_gender',
                        cfg.DATA.ATTENTION_DIR, 'data/COCO',
    ...
    def load_filenames_labels(self,coco, split):
        ...
        if split == 'train':
            train_ids = np.loadtxt(
                ...
                os.path.join(self.root, 'COCO_gender', 'biased_split', 'train.ids.prob.txt'),
                dtype=float
            )
            ...
            val_man   = np.loadtxt(
                ...
                os.path.join(self.root, 'COCO_gender', 'balanced_split', 'man_imgs_final3.txt'),
                dtype=float
            )
            val_woman = np.loadtxt(
                #os.path.join(self.root, 'COCO_gender', 'balanced_split', 'val_woman.txt'),
                os.path.join(self.root, 'COCO_gender', 'balanced_split', 'woman_imgs_final3.txt'),
                dtype=float
            )
            ...
            test_man   = np.loadtxt(
                ...
                os.path.join(self.root, 'COCO_gender', 'balanced_split', 'test_man_imgs_final.txt'),
                dtype=float
            )
            test_woman = np.loadtxt(
                #os.path.join(self.root, 'COCO_gender', 'balanced_split', 'test_woman.txt'),
                os.path.join(self.root, 'COCO_gender', 'balanced_split', 'test_woman_imgs_final.txt'),
                dtype=float
            )
