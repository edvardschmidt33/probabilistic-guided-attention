...
class CocoCaptionsCap(Dataset):
    ...
    """
    def __init__(self, flag, root, annFile, ids=None,
        ...
        if flag == 'val':
            instances_file_path = '../Datasets/Microsoft-COCO/annotations/instances_val2014.json'
            output_ids = '../ProbVLM/Datasets/coco/filtered_ids_val3.txt'
        elif flag == 'train':
            instances_file_path = '../Datasets/Microsoft-COCO/annotations/instances_train2014.json'
            output_ids = '../ProbVLM/Datasets/coco/filtered_ids_train3.txt'
        else:
            instances_file_path = '../Datasets/Microsoft-COCO/annotations/instances_val2014.json'
            output_ids = '../ProbVLM/Datasets/coco/filtered_ids_test3.txt'

        # Initialize COCO API for instance annotations
        coco = COCO(instances_file_path)

        # Get the category ID for "person"
        person_category_id = coco.getCatIds(catNms=['person'])[0]  # 'person' corresponds to category ID 1
        ...
        human_images = coco.loadImgs(image_ids)
        human_image_ids = [image['id'] for image in human_images]
        #print(len(human_image_ids))
        
        # Initialize a new list for filtered IDs
        self.filtered_ids = []
        
        # Iterate over ids and check the category_id for each annotation
        for annotation_id in self.ids:
            annotation = self.coco.loadAnns(int(annotation_id))#[0]
            image_id = annotation[0]['image_id']
            if image_id in human_image_ids:
                self.filtered_ids.append(annotation_id) #adding the annotation id for the filtered images

        self.all_image_ids = self.filtered_ids  # Update self.ids to only contain filtered IDs

        #print('Filtered IDs of the captions:', len(self.all_image_ids), flush=True)
        
        self.transform = transform
        self.target_transform = target_transform

        #print(self.ids[0], flush=True)
        #print(self.coco.loadAnns(self.ids[0])[0]['image_id'], flush=True)
        #self.all_image_ids = set([self.coco.loadAnns(annotation_id)[0]['image_id'] for annotation_id in self.ids])
        #print(self.all_image_ids[:10], flush=True)
        ...
        if 'val' in annFile:
            self.ids = self.filtered_ids#[:5000]
        else: 
            self.ids = self.filtered_ids
            
        with open(output_ids, 'w') as file:
            for img_id in sorted(self.ids):  # Sort the IDs for consistent output
                file.write(f"{img_id}\n")
                
        self.n_images = len(self.all_image_ids)
        print("Lenght of ids: ",self.ids)
    ...
    def __getitem__(self, index, get_caption=False):
        ...
        annotation = coco.loadAnns(int(annotation_id))[0]
